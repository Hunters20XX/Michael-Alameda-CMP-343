 ...
 function markTaskComplete(button) {
    ...
    const taskName = row.cells[0].innerText; // Get the task name
 ...
 function removeTask(button) {
    const row = button.closest('tr');
    const taskName = row.cells[0].innerText; // Assuming task name is in the first cell
    ...
    function render() {
        ...
        taskTable.innerHTML = tasks.map(task => `
            <tr>
                ...
                <td style="${task.completed ? 'text-decoration: line-through;' : ''}">${task.deadline}</td>
